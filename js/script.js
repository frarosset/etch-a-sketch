...
function roundToPrecision(number, precision = 0) {
    const exp = Math.pow(10, precision);
    return Math.round(number * exp) / exp;
}

function NumberToAngle(number){
    // round to the 10 decimal position, to avoid numeric errors,
    // which give, e.g., 34.550000000000004 instead of 34.55
    return roundToPrecision(number,10) + '°';
    //return Math.round(number) + '°';
}

function AngleToNumber(string){
...
function PercentageToNumber(string){
    return Number(string.replace('%', ''));
}
...
let percentageOption = {
    ...
    format: {
        to: NumberToPercentage,
        from: PercentageToNumber
    },
    tooltips: true
}
